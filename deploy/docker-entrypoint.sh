#!/bin/sh
# tablinum container entrypoint: prepare /data and the git identity, then exec.
set -eu

CONTENT_DIR="${TABLINUM_CONTENT_DIR:-/data/content}"
...
git config --global --get user.name  >/dev/null 2>&1 || \
  git config --global user.name  "${TABLINUM_GIT_AUTHOR_NAME:-tablinum}"
git config --global --get user.email >/dev/null 2>&1 || \
  git config --global user.email "${TABLINUM_GIT_AUTHOR_EMAIL:-tablinum@localhost}"
git config --global init.defaultBranch "${TABLINUM_GIT_BRANCH:-main}" >/dev/null 2>&1 || true

# An SSH remote needs the host key up front, otherwise the first pull hangs on a
# prompt that nobody can answer.
if [ -n "${TABLINUM_SSH_KNOWN_HOSTS:-}" ]; then
  mkdir -p "$SSH_DIR"
  chmod 700 "$SSH_DIR"
  printf '%s\n' "$TABLINUM_SSH_KNOWN_HOSTS" > "$SSH_DIR/known_hosts"
...
if [ "$#" -eq 2 ] && [ "$1" = "node" ] && [ ! -f "$2" ]; then
  ...
  if [ -z "$found" ]; then
    echo "tablinum: no server entry point found. Looked for $2 and the dist/ fallbacks." >&2
    exit 1
  fi
  set -- node "$found"
fi
